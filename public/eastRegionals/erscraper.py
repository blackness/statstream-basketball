...
def has_real_performance_value(headers, results):
    if not headers or not results:
        return False

    preferred_columns = [
        "Time", "Mark", "Result", "Performance", "Distance", "Height"
    ]

    present_preferred = [h for h in headers if h in preferred_columns]

    if present_preferred:
        for row in results:
            for col in present_preferred:
                val = row.get(col, "")
                if not is_placeholder_value(val):
                    return True
        return False

    ignored_columns = {"Place", "Name", "Bib", "School", "Team", "Heat", "Lane"}
    for row in results:
...
:root {{
  ...
  --bronze:rgba(180,83,9,.18); --ok:#16a34a; --dim:#475569; --chg:#2563eb; --partial:#ca8a04;
}}
...
.status-partial {{ border-color: var(--partial); color: #fde68a; }}
...
<header>
  ...
  <div class="stat-row">
    ...
    <div class="stat"><div class="stat-label">Partial</div><div class="stat-value">{summary_counts['partial']}</div></div>
...
def upload_snapshot_to_supabase(all_races, flat_rows, last_poll_at, summary_counts):
    if not SUPABASE_URL or not SUPABASE_SERVICE_ROLE_KEY:
        log_line("Supabase credentials not set; skipping upload")
        return
    ...
    try:
        ...
        if resp.ok:
            log_line("Uploaded snapshot to Supabase")
        else:
            log_line(f"Supabase upload failed: {resp.status_code} {resp.text}")
    except Exception as e:
        log_line(f"Supabase upload error: {e}")
...
def extract_race_page(browser, url: str):
    ...
    has_real_performance = has_real_performance_value(headers, results)

    if not has_rows:
        status = "not_posted"
    elif has_real_performance:
        status = "posted"
    else:
        status = "partial"

    rows_hash = make_rows_hash(headers, results) if has_rows else ""

    log_line(f"SCRAPED: {race_name} status={status} rows={len(results)}")
...
def merge_with_state(old_state, scraped_race, checked_at):
    ...
    if scraped_race["status"] in {"partial", "posted"}:
        if previous_status != scraped_race["status"]:
            just_changed = True
...
def build_summary_counts(all_races):
    ...
    partial = sum(1 for r in all_races if r.get("status") == "partial")
    not_posted = sum(1 for r in all_races if r.get("status") == "not_posted")
...
def one_poll_cycle(browser, state):
    ...
    for i, race in enumerate(race_links, 1):
        ...
        try:
            ...
            if merged["just_changed"]:
                if merged["previous_status"] == "not_posted" and merged["status"] == "partial":
                    log_line(f"NOW PARTIAL: {merged['race_name']} ({merged['url']})")
                elif merged["previous_status"] != "posted" and merged["status"] == "posted":
    ...
    upload_snapshot_to_supabase(all_races, flat_rows, checked_at, summary_counts)

    log_line(
        f"Poll complete: total={summary_counts['total']} posted={summary_counts['posted']} "
        f"partial={summary_counts['partial']} not_posted={summary_counts['not_posted']} "
        f"changed={summary_counts['changed']}"
    )
