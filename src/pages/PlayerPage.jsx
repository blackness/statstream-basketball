...
export default function PlayerPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const { data: t, error: te } = await supabase
        .from('teams').select('*, players:players(*)').eq('id', p.team_id).single();
      if (te) throw te;
      setTeam({ ...t, roster: t.players || [] });

      const { data: g, error: ge } = await supabase
        .from('games').select('*')
        .eq('team_id', p.team_id).eq('status', 'completed')
        .order('created_at', { ascending: false });
      if (ge) throw ge;
      setGames((g || []).filter(game => game.stats?.[id]));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  if (!player || !team) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        <p className="font-black text-gray-400 text-lg">Player not found</p>
        <button onClick={() => navigate('/')} className="mt-4 text-blue-600 font-bold text-sm">← Home</button>
      </div>
    </div>
  );
  ...
  }, { pts:0,fgm:0,fga:0,tpm:0,tpa:0,ftm:0,fta:0,oreb:0,dreb:0,ast:0,stl:0,blk:0,min:0 });

  const reb  = totals.oreb + totals.dreb;
  const avgs = {
    pts: avg(totals.pts,gp), reb: avg(reb,gp), ast: avg(totals.ast,gp),
    stl: avg(totals.stl,gp), blk: avg(totals.blk,gp), min: avg(totals.min,gp),
  };

  const careerHighs = games.reduce((h, g) => {
    const s = g.stats[player.id] || {};
    const r = (s.oreb||0)+(s.dreb||0);
    return {
      pts: h.pts.val>=(s.pts||0) ? h.pts : { val:s.pts||0, game:g },
      reb: h.reb.val>=r          ? h.reb : { val:r,         game:g },
      ast: h.ast.val>=(s.ast||0) ? h.ast : { val:s.ast||0,  game:g },
    };
  }, { pts:{val:0,game:null}, reb:{val:0,game:null}, ast:{val:0,game:null} });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4 pb-16">
          ...
          {TABS.map(t => (
            <button
              ...
              className={`flex-1 py-2 rounded-lg font-bold text-sm transition ${
                tab === t.key ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-400 hover:bg-gray-50'
              }`}
            >
              {t.label}
            </button>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-px bg-gray-100">
                {[
                  { label:'FG%',  pct:fmtPct(totals.fgm+totals.tpm,totals.fga+totals.tpa), made:totals.fgm+totals.tpm, att:totals.fga+totals.tpa },
                  { label:'3P%',  pct:fmtPct(totals.tpm,totals.tpa), made:totals.tpm, att:totals.tpa },
                  { label:'FT%',  pct:fmtPct(totals.ftm,totals.fta), made:totals.ftm, att:totals.fta },
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-black text-gray-900 text-sm mb-3">Career Highs</h3>
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={key} className="text-center bg-gray-50 rounded-xl p-3">
                    <p className="text-3xl font-black text-blue-600">{careerHighs[key].val}</p>
                    <p className="text-[10px] text-gray-400 font-bold mt-1">{label}</p>
                    {careerHighs[key].game && (
                      <p className="text-[9px] text-gray-300 mt-0.5 truncate">
                        vs {careerHighs[key].game.opponent}
                      </p>
                    )}
                  </div>
                ))}
              ...
              <table className="text-xs min-w-max w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    {['PTS','REB','AST','STL','MIN','FG%'].map(h => (
                      <th
                        key={h}
                        className={`py-3 px-3 text-right font-bold ${h==='PTS' ? 'text-blue-600' : 'text-gray-500'}`}
                      >
                        {h}
                      </th>
                    ))}
                      ...
                      <tr key={game.id} className={rowBg}>
                        <td className={`py-2.5 px-4 sticky left-0 ${rowBg}`}>
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="font-bold text-gray-900 truncate max-w-[85px]">vs {game.opponent}</p>
                              <p className="text-[10px] text-gray-400">{fmtDt(game.created_at)}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-2.5 px-3 text-right font-black text-blue-600 tabular-nums">{s.pts||0}</td>
                        <td className="py-2.5 px-3 text-right text-gray-700 tabular-nums">{r}</td>
        ...
        {tab === 'highs' && gp > 0 && (
          <div className="space-y-3">
            {[
              ...
              const isWin  = ours > opp;
              return (
                <div key={key} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-4">
                  <div className="flex-1 min-w-0">
