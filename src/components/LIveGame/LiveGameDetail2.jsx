...
// ── Main Component ────────────────────────────────────────────────────────────
const LiveGameDetail = ({ initialGame, team, onBack }) => {
          ...
          <div className="grid grid-cols-3 gap-4 items-center mb-3">
            ...
            <div className="text-center">
              <div className="text-xs font-black text-white/40 uppercase tracking-wide mb-1 truncate">
                {isHome ? game.opponent : team?.name}
              </div>
            ...
            <div className="text-center">
              <div className="text-xs font-black text-white/40 uppercase tracking-wide mb-1 truncate">
                {isHome ? team?.name : game.opponent}
              </div>
              <div className="text-6xl font-black text-white tabular-nums leading-none">
                {myScore}
              </div>
          ...
          {myScore !== oppScore && (
            <div className="text-center">
              <span className="text-[10px] font-bold text-white/30">
                {myScore > oppScore ? team?.name : game.opponent} {' '}
                leads by {Math.abs(myScore - oppScore)}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        {plays.length > 0 && (
          <div className="bg-gray-900 rounded-xl overflow-hidden border border-white/5">
            <div className="px-4 py-3 border-b border-white/5 flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">Play by Play</span>
            </div>
            <div className="divide-y divide-white/5">
              {plays.slice(0, 5).map((play, i) => (
                <div key={play.id || i} className={`flex items-center justify-between px-4 py-2.5 ${i === 0 ? 'bg-white/5' : ''}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${play.team === 'home' ? 'bg-blue-400' : 'bg-red-400'}`} />
                    <span className={`text-sm font-medium ${i === 0 ? 'text-white' : 'text-white/50'}`}>
                      {play.description}
                    </span>
                  </div>
                  <span className="text-[10px] font-bold text-white/25 tabular-nums flex-shrink-0 ml-4">
                    Q{play.period} {play.time}
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="bg-gray-900 rounded-xl overflow-hidden border border-white/5">
          <div className="px-4 py-3 border-b border-white/5">
            <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{team?.name} — Box Score</span>
          </div>
