...
const LiveGameDetail = ({ initialGame, team: initialTeam, onBack }) => {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      const { data, error } = await supabase
        .from('games').select('*').eq('id', initialGame.id).single();
      if (data && !error) setGame(data);
    };
    fetchGame();
  }, [initialGame.id]);

  // Fetch roster — always fetch fresh from DB
  useEffect(() => {
  ...
  // isHome: game.home_team stores the actual home team name
  // Compare against initialTeam name, or against opponent (if home_team === opponent, we're away)
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-gray-900 rounded-xl overflow-hidden border border-white/5">
          <div className="px-4 py-3 border-b border-white/5">
            <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{myName} — Box Score</span>
          </div>
