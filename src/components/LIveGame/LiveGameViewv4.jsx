...
const LiveGameView = ({ 
  ...
  // Offline support
  const { isOnline, isSyncing, queueLength, saveWithFallback } = useOfflineQueue();

  // Factory — always pass saveWithFallback so saves work offline
  const createManager = (gameId) => new GameStateManager(gameId, saveWithFallback);
  ...
  }, [isTimerRunning, currentGameId, gameTime, homeScore, awayScore, currentPeriod, liveStats, opponentStats, activePlayers, plusMinus, homeFouls, awayFouls]);
  ...
  if (gamePhase === 'selectStarters') {
    ...
    const handleBeginGame = async () => {
      if (!navigator.onLine) {
        toast?.error('Internet required to start a new game. Connect and try again.');
        return;
      }
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto px-4 py-6">
          ...
          {!isOnline && (
            <div className="mb-3 px-4 py-3 bg-orange-50 border border-orange-200 rounded-xl text-sm text-orange-700 font-medium text-center">
              📵 No internet — connect to start a new game
            </div>
          )}
          ...
          <button
            ...
            disabled={!isOnline}
            className={`w-full py-4 rounded-xl font-black text-lg uppercase tracking-wide transition shadow-sm ${
              isOnline
                ? 'bg-green-600 hover:bg-green-700 active:bg-green-800 text-white'
                : 'bg-gray-200 text-gray-400 cursor-not-allowed'
            }`}
          >
            {isOnline ? 'Start Game' : 'No Internet Connection'}
          </button>
        </div>
      </div>
    );
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-50">
        ...
        {(!isOnline || isSyncing || queueLength > 0) && (
          <div className={`px-4 py-1.5 text-xs font-bold text-center ${
            ...
            {!isOnline
              ? `📵 Offline — stats saving locally${queueLength > 0 ? ` (${queueLength} pending)` : ''}`
              : isSyncing
                ? `⏫ Syncing ${queueLength} saved ${queueLength === 1 ? 'stat' : 'stats'}...`
                : '✓ All stats synced'
            }
          </div>
        )}
