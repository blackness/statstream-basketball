...
const LiveGameView = ({ 
    ...
    const initializeGame = async () => {
      ...
      if (existingGame) {
        ...
        if (gameData) {
          ...
          setGameTime(gameData.time_remaining || (gameSettings.periodLength * 60));
  ...
  // Autosave clock every 10 seconds while timer is running
  useEffect(() => {
    if (!isTimerRunning || !currentGameId) return;
  ...
  }, [isTimerRunning, currentGameId, gameTime, homeScore, awayScore, currentPeriod, liveStats, opponentStats, activePlayers, plusMinus, homeFouls, awayFouls, recentPlays, playLog]);
  ...
  const handleQuickStat = async (playerId, statType, points = 0, missed = false) => {
    ...
    // Build the new play entry before saving so it gets persisted
    const statName = {
    ...
    const newPlay = {
      id: Date.now(),
      description: `${player?.name} - ${statName}`,
      time: `${Math.floor(gameTime / 60)}:${(gameTime % 60).toString().padStart(2, '0')}`,
      period: currentPeriod,
      points,
      missed,
  ...
  const handleOpponentQuickStat = async (statType, points = 0, missed = false) => {
    ...
    // Opponent is away when we're home, home when we're away
    const newHomeScore = gameSettings.isHome ? homeScore : homeScore + points;
    const newAwayScore = gameSettings.isHome ? awayScore + points : awayScore;
    ...
    const opponentPlay = {
      id: Date.now(),
      description: `${gameSettings.opponent} - ${{
  ...
  const saveEditedScore = async () => {
    ...
    const manager = new GameStateManager(currentGameId);
    await manager.saveGameState({ homeScore: newHome, awayScore: newAway, period: currentPeriod, timeRemaining: gameTime, timerRunning: isTimerRunning, stats: liveStats, opponentStats, activePlayers, plusMinus, gameSettings, recentPlays, playLog });
  };
  ...
  const saveEditedClock = async () => {
    ...
    const manager = new GameStateManager(currentGameId);
    await manager.saveGameState({ homeScore, awayScore, period: currentPeriod, timeRemaining: newTime, timerRunning: isTimerRunning, stats: liveStats, opponentStats, activePlayers, plusMinus, gameSettings, recentPlays, playLog });
  };
  ...
  const undoStat = async (playerId, statType, points, wasMissed) => {
    ...
    let newHomeScore = gameSettings.isHome ? homeScore - points : homeScore;
    let newAwayScore = gameSettings.isHome ? awayScore : awayScore - points;
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4 items-center mb-2 sm:mb-3">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1 sm:gap-2">
                ...
                {awayFouls}
                {awayFouls >= 4 && (
                  <span className="inline-flex items-center justify-center w-4 h-4 sm:w-5 sm:h-5 bg-red-600 text-white text-xs font-black rounded-full">B</span>
                )}
              </div>
            </div>
              ...
              <div className="bg-white rounded-xl p-2 sm:p-3 shadow-sm border border-gray-200">
                ...
                {showSubPanel && (
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    ...
                    {/* Bench players */}
                    {team.roster?.filter(p => !activePlayers.includes(p.id) && !notPresent.includes(p.id)).length > 0 && (
                    ...
                    {/* DNT late arrivals */}
                    {notPresent.filter(id => team.roster?.find(p => p.id === id)).length > 0 && (
                      <div>
                        <p className="text-xs font-bold text-gray-400 uppercase mb-1.5">Late Arrival</p>
                          ...
                          {notPresent.map(id => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  if (selectedPlayer && activePlayers.includes(selectedPlayer)) {
                                    // Remove from notPresent first
                                    setNotPresent(prev => prev.filter(i => i !== id));
                                    handleSwapPlayers(selectedPlayer, player.id);
                                    setShowSubPanel(false);
                                  }
                                }}
                                disabled={!selectedPlayer || !activePlayers.includes(selectedPlayer)}
                                className={`p-2 rounded-lg font-bold text-left transition-all border border-dashed ${
                                  !selectedPlayer || !activePlayers.includes(selectedPlayer)
                                    ? 'bg-gray-50 border-gray-200 text-gray-300 cursor-not-allowed'
                                    : 'bg-yellow-50 border-yellow-300 hover:bg-yellow-100 text-gray-900 active:scale-95'
                                }`}
                              >
                                <div className="text-xs text-gray-400">#{player.number}</div>
                                <div className="text-sm font-black truncate">{player.name}</div>
                                <div className="text-xs text-yellow-600 font-bold">DNT</div>
                              </button>
                            );
                          })}
                        </div>
      ...
      <div className="max-w-4xl mx-auto px-2 sm:px-4 pb-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-blue-600 px-4 py-2">
            <h3 className="font-black text-white text-sm uppercase tracking-wide">Live Box Score</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-gray-100">
                {team.roster
                  ?.filter(p => liveStats[p.id])
                  .sort((a, b) => (liveStats[b.id]?.pts || 0) - (liveStats[a.id]?.pts || 0))
                  .map(player => {
                    const s = liveStats[player.id] || {};
                    const fgm = (s.fgm || 0) + (s.tpm || 0);
                    const fga = (s.fga || 0) + (s.tpa || 0);
                    const reb = (s.oreb || 0) + (s.dreb || 0);
                    const pm = plusMinus[player.id] || 0;
                    return (
                      <tr key={player.id} className={`hover:bg-gray-50 ${activePlayers.includes(player.id) ? 'bg-blue-50/40' : ''}`}>
                        ...
                        <td className="px-2 py-2 text-center tabular-nums text-gray-600">{s.ftm || 0}/{s.fta || 0}</td>
                        <td className="px-2 py-2 text-center tabular-nums">{reb}</td>
                        <td className="px-2 py-2 text-center tabular-nums">{s.ast || 0}</td>
                        <td className="px-2 py-2 text-center tabular-nums">{s.stl || 0}</td>
                        <td className="px-2 py-2 text-center tabular-nums">{s.blk || 0}</td>
                        <td className="px-2 py-2 text-center tabular-nums">{s.to || 0}</td>
                        <td className="px-2 py-2 text-center tabular-nums">{s.pf || 0}</td>
                        <td className={`px-2 py-2 text-center font-bold tabular-nums ${pm > 0 ? 'text-green-600' : pm < 0 ? 'text-red-600' : 'text-gray-400'}`}>
                          {pm > 0 ? '+' : ''}{pm}
                        </td>
                      </tr>
                    );
              ...
              {Object.keys(liveStats).length > 0 && (() => {
                const totals = Object.values(liveStats).reduce((acc, s) => ({
                  pts: acc.pts + (s.pts || 0),
                  fgm: acc.fgm + (s.fgm || 0) + (s.tpm || 0),
                  fga: acc.fga + (s.fga || 0) + (s.tpa || 0),
                  tpm: acc.tpm + (s.tpm || 0),
                  tpa: acc.tpa + (s.tpa || 0),
                  ftm: acc.ftm + (s.ftm || 0),
                  fta: acc.fta + (s.fta || 0),
                  reb: acc.reb + (s.oreb || 0) + (s.dreb || 0),
                  ast: acc.ast + (s.ast || 0),
                  stl: acc.stl + (s.stl || 0),
                  blk: acc.blk + (s.blk || 0),
                  to: acc.to + (s.to || 0),
                ...
                return (
                  <tfoot>
                    <tr className="bg-gray-50 border-t-2 border-gray-300 font-black text-[10px] uppercase">
                      ...
                      <td className="px-2 py-2 text-center tabular-nums text-gray-600">{totals.ftm}/{totals.fta}</td>
                      <td className="px-2 py-2 text-center tabular-nums">{totals.reb}</td>
                      <td className="px-2 py-2 text-center tabular-nums">{totals.ast}</td>
                      <td className="px-2 py-2 text-center tabular-nums">{totals.stl}</td>
